...
server <- function(input, output, session) {
  ...
  output$plot_monthly <- renderPlotly({
    # FIX: urutkan by month_num dulu, paksa urutan bulan
    df <- monthly_2023 %>% arrange(month_num) %>%
      mutate(month = factor(month, levels = month.abb))
    
    p <- plot_ly(df, x=~month, y=~export, name="Export",
                 type="scatter", mode="lines+markers",
      ...
      layout(
        ...
        xaxis=list(title="", categoryorder="array",
                   categoryarray=month.abb),  # ← kunci fix-nya
