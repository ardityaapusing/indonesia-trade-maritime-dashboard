...
annual_trade <- data.frame(
  year    = 2018:2023,
  export  = c(180.2, 167.7, 163.2, 231.6, 291.9, 258.8),
  import  = c(188.7, 170.7, 141.6, 196.2, 237.7, 221.9),
  stringsAsFactors = FALSE
) %>%
  mutate(
    balance       = export - import,
    export_growth = c(NA, round(diff(export) / head(export, -1) * 100, 1)),
    import_growth = c(NA, round(diff(import) / head(import, -1) * 100, 1))
  )
